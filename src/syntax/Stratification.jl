...
struct SFNames
  ...
  # index -> new index, where the first index is the actual index of the vector,
  # the second is the int at that location
  mvs::Vector{Int}
  mvsv::Vector{Int}
  mvv::Vector{Int}
  mvf::Vector{Int}
  ...
  SFNames(sfarg::AbstractStockAndFlowF) = (new(sfarg,
   ...
   state_dict(snames(sfarg)), state_dict(svnames(sfarg)), 
    state_dict(vnames(sfarg)), state_dict(fnames(sfarg)), state_dict(pnames(sfarg)),
...
function all_unique_names(sfn::SFNames)::Bool
  return (allunique(sfn.snames) && allunique(sfn.svnames) && allunique(vnames) 
    && allunique(fnames) && allunique(pnames))
end

function no_temp_strat_default_in_names(sfn::SFNames, temp_strat_default)::Bool
  return (temp_strat_default ∉ keys(sfn.s) && temp_strat_default ∉ keys(sfn.sv) 
    && temp_strat_default ∉ keys(sfn.v) && temp_strat_default ∉ keys(sfn.f) 
    && temp_strat_default ∉ keys(sfn.p))
end
...
"""
  interpret_stratification_standard_notation(mapping_pair::Expr)::Tuple{Vector{DSLArgument},
Vector{DSLArgument}} Take an expression of the form a1, ..., => t <= s1, ...,
where every element is a symbol, and return a 2-tuple of form ((a1 => t, a2 =>
t, ...), (s1 => t, ...))
"""
...
function interpret_stratification_generalized_notation(mapping_pair::Expr)::Vector{Vector{DSLArgument}}
  ...
  return ([((typeof(tup) == Expr) && (tup.head == :tuple)) ? 
    [DSLArgument(sym, type) for sym ∈ tup.args] : [DSLArgument(tup, type)] for tup ∈ other])
end
...
"""
Gets mapping information from each line and updates dictionaries.  If a symbol
already has a mapping and another is found, keep the first, or throw an error if
strict_matches = true.
"""
function read_stratification_line_and_update_dictionaries!(line::Expr, other_names::Vector{Dict{Symbol, Int}}, type_names::Dict{Symbol, Int}, other_mappings::Vector{Dict{Int, Int}} ; use_standard_stratification_syntax = true, strict_matches = false, use_flags = true)
  if use_standard_stratification_syntax
    ...
    # need to do this here, since we know the number of other_mappings at this
    # point, but not in the interpret_stratification_notation
    @assert length(line.args) == 3 
    @assert typeof(line.args[3]) == Symbol
    @assert line.args[1] == :(=>)
    @assert length(line.args[2].args) == length(other_names)
    @assert all(tup -> typeof(tup) == Symbol || tup.args[1] == :~ 
      || tup.head == :tuple, line.args[2].args) # every element of the vector is an expression of tuple.
    @assert all(tup -> typeof(tup) == Symbol || tup.args[1] == :~ 
      || all(sym -> typeof(sym) <: Union{Symbol, Expr}, tup.args), 
      line.args[2].args) # ensure all arguments in the tuples are expressions or symbols.
    # In the future, if we have additional flags, may need to check for them as
    # well. These asserts are a bit sloppy
  end
...
"""
Print all symbols such that the corresponding int is 0, representing an unmapped
object.
"""
...
"""
Iterates over each line in a stratification syntax block and updates the
appropriate dictionaries.
"""
function iterate_over_stratification_lines!(block, other_names::Vector{SFNames},
   type_names::SFNames; use_standard_stratification_syntax=true, 
   strict_matches=false, use_flags=true)
  
  current_phase = (_, _) -> ()
  for statement in block.args
    @match statement begin
      QuoteNode(:stocks) => begin
        current_phase = s -> read_stratification_line_and_update_dictionaries!(s, 
        (getfield.(other_names, :s))::Vector{Dict{Symbol, Int}}, type_names.s, 
        (getfield.(other_names, :ms))::Vector{Dict{Int, Int}}; 
        use_standard_stratification_syntax=use_standard_stratification_syntax, 
        strict_matches=strict_matches, use_flags=use_flags)
      end
      QuoteNode(:sums) => begin
        current_phase = sv -> read_stratification_line_and_update_dictionaries!(sv,
          (getfield.(other_names, :sv))::Vector{Dict{Symbol, Int}}, type_names.sv,
          (getfield.(other_names, :msv))::Vector{Dict{Int, Int}}; 
          use_standard_stratification_syntax=use_standard_stratification_syntax, 
          strict_matches=strict_matches, use_flags=use_flags)
      end
      QuoteNode(:dynamic_variables) => begin
        current_phase = v -> read_stratification_line_and_update_dictionaries!(v,
          (getfield.(other_names, :v))::Vector{Dict{Symbol, Int}}, type_names.v,
          (getfield.(other_names, :mv))::Vector{Dict{Int, Int}};
          use_standard_stratification_syntax=use_standard_stratification_syntax,
          strict_matches=strict_matches, use_flags=use_flags)
      end
      QuoteNode(:flows) => begin
        current_phase = f -> read_stratification_line_and_update_dictionaries!(f,
         (getfield.(other_names, :f))::Vector{Dict{Symbol, Int}}, type_names.f, 
         (getfield.(other_names, :mf))::Vector{Dict{Int, Int}}; 
         use_standard_stratification_syntax=use_standard_stratification_syntax, 
         strict_matches=strict_matches, use_flags=use_flags)
      end
      QuoteNode(:parameters) => begin
        current_phase = p -> read_stratification_line_and_update_dictionaries!(p,
         (getfield.(other_names, :p))::Vector{Dict{Symbol, Int}}, type_names.p,
          (getfield.(other_names, :mp))::Vector{Dict{Int, Int}}; 
          use_standard_stratification_syntax=use_standard_stratification_syntax,
           strict_matches=strict_matches, use_flags=use_flags)
      end
      QuoteNode(kw) =>
...
"""
Apply default mappings, infer mapping if there's only a single option, and
convert from Dict{Int, Int} to Vector{Int}
"""
function complete_mappings!(sfm::SFNames, sftype::SFNames; strict_mappings = false)
  ...
  if !strict_mappings # if there is only one stock, it needs to have index 1
    one_type_stock = length(sftype.snames) == 1 ? 1 : 0 
  ...
  stock_mappings::Vector{Int} = [get(all_index_mappings[1], i, 
    max(default_index_stock, one_type_stock))  for i ∈ eachindex(sfm.snames)]
  sum_mappings::Vector{Int} =  [get(all_index_mappings[2], i,  
    max(default_index_sum, one_type_sum))  for i ∈ eachindex(sfm.svnames)]
  dyvar_mappings::Vector{Int} = [get(all_index_mappings[3], i, 
    max(default_index_dyvar, one_type_dyvar))  for i ∈ eachindex(sfm.vnames)]
  flow_mappings::Vector{Int} =  [get(all_index_mappings[4], i, 
    max(default_index_flow, one_type_flow))  for i ∈ eachindex(sfm.fnames)]
  param_mappings::Vector{Int} = [get(all_index_mappings[5], i,  
    max(default_index_param, one_type_param))  for i ∈ eachindex(sfm.pnames)]
...
"""
  ...
  1. Grab all names from strata, type and aggregate, and create dictionaries
     which map them to their indices
  2. Iterate over each line in the block 2a. Split each line into a dictionary
    which maps all strata to that type and all aggregate to that type 2b.
    Convert from two Symbol => Symbol dictionaries to two Int => Int
      dictionaries, using the dictionaries from step 1 2bα. If applicable, for
    symbols with ~ as a prefix, find all symbols with matching substrings in the
    symbol dictionaries, and map all those 2c. Accumulate respective
    dictionaries (optionally, only allow first match vs throw an error
    (strict_matches = false vs true))
  3. Create an array of 0s for stocks, flows, parameters, dyvars and sums for
    strata and aggregate.   Insert into arrays all values from the two Int =>
    Int dictionaries 3a. If strict_mappings = false, if there only exists one
    option in type to map to, and it hasn't been explicitly specified, add it.
    If strict_mappings = true and it hasn't been specified, throw an error.
  4. Do a once-over of arrays and ensure there aren't any zeroes (unmapped
     values) remaining (helps with debugging when you screw up stratifying)
  5. Deal with attributes (create a copy of type sf with attributes mapped to
     nothing)
  6. Infer LS, LSV, etc.
  7. Construct strata -> type and aggregate -> type ACSetTransformations
  8. Return pullback (with flattened attributes)
"""
function sfstratify(others::Vector{K}, type::K, block::Expr 
  ; use_standard_stratification_syntax = true, strict_mappings = false, 
  strict_matches = false, temp_strat_default = :_, use_temp_strat_default = true, 
  use_flags = true, return_homs = false) where {K <: AbstractStockAndFlowStructureF}
  ...
  if use_temp_strat_default
    ...
    @assert (all((sfn -> no_temp_strat_default_in_names(sfn, temp_strat_default)).(other_names))
      && no_temp_strat_default_in_names(type_names, temp_strat_default))
      "A stockflow contains $(temp_strat_default) !  \
      Please change temp_strat_default to a different symbol or rename offending object."
    (sfn -> add_temp_strat_default!(sfn, temp_strat_default)).(other_names)
  end


  # STEP 2
  iterate_over_stratification_lines!(block, other_names, type_names ; 
    use_standard_stratification_syntax=use_standard_stratification_syntax, 
    strict_matches=strict_matches, use_flags=use_flags)
  ...
  # This bit makes debugging when making a stratification easier.  Tells you
  # exactly which ones you forgot to map.
  ...
  # STEP 5 NothingFunction(x...) = nothing;
  no_attribute_type = map(type, Name=NothingFunction, 
    Op=NothingFunction, Position=NothingFunction)

  # STEP 6/7 This is where we pull out the magic to infer links.
  #
  #  A <- C -> B 
  #  ||       || 
  #  v         v 
  ...
  generate_all_mappings_function = m -> Dict(infer_links(m.sf, type, 
    get_mappings_infer_links_format(m))..., 
    get_mappings_infer_links_format(m)..., 
    :Op => NothingFunction, :Position => NothingFunction, 
    :Name => NothingFunction)

  all_mappings = generate_all_mappings_function.(other_names)
   
  all_transformations = [
    ACSetTransformation(sfn.sf, no_attribute_type ; mappings...) 
    for (sfn, mappings) ∈ zip(other_names, all_mappings)
  ]

  # STEP 8

  pullback_model = pullback(all_transformations) |> apex |> 
...
"""
  stratify(strata, type, aggregate, block) Take three stockflows and a block
describing where the first and third map on to the second, and get a new
stratified stockflow. Left side are strata objects, middle are type, right are
aggregate.  Each strata and aggregate object is associated with one type object.
The resultant stockflow contains objects which are the product of strata and
aggregate objects which map to the same type object. Use _ to match all objects
in that category, ~ as a prefix to match all objects with the following string
as a substring.  Objects always go with their first match. If the type model has
a single object in a category, the mapping to it is automatically assumed.  In
the below example, we wouldn't need to specify :stocks or :sums.
...
macro stratify(strata, type, aggregate, block)
  ...
  quote
    sfstratify([$(esc(strata)), $(esc(aggregate))],
      $(esc(type)), $(esc(escaped_block)))
  end
end


"""
Alternate syntax for stratification, allows for an arbitrary number of
stockflows in a pullback. Second last argument must be the type stockflow, last
must be the block describing how the stratificaition is done.  All arguments
before that must be stockflows.
...
macro n_stratify(args...)
  if length(args) < 3
    return :(MethodError("Too few arguments provided!  Please provide some \
      number of stockflows, then the type stock flow, then a quote block."))
  else
    escaped_block = Expr(:quote, args[end])
    other_sfs = esc.(args[1:end-2])
    type = (esc(args[end-1]))
    quote
      sfstratify([$(other_sfs...)], $type, $escaped_block ;
        use_standard_stratification_syntax = false)
    end
  end
end
